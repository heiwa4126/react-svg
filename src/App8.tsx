...
const SvgTextComponent = ({ texts, gap = 40 }: { texts: string[]; gap?: number }) => {
	...
	const [state, setState] = useState<{ yPositions: number[]; isRendered: boolean }>({
		yPositions: new Array(texts.length).fill(0),
		isRendered: false,
	});
	...
	useEffect(() => {
		...
		const offsetY = state.yPositions[0] - bboxes[0].y;
		for (const bbox of bboxes) {
			if (bbox) {
				yPosArray.push(bbox.y + offsetY);
			}
		}
		setState({ yPositions: yPosArray, isRendered: true });
	}, []);
	...
	return (
		<svg width={w} height={h} viewBox={`0 0 ${w * s} ${h * s}`}>
			...
			<g visibility={state.isRendered ? "visible" : "hidden"}>
				{texts.map((text, index) => (
					<text
						...
						y={state.yPositions[index]} // 計算したy座標を適用
						textAnchor="left"
						fontSize={16 * s}
						dy="1em"
					>
...
function App() {
	...
	return (
		<>
			<h1>8. 重なるテキストをずらす(2) ちらつきをおさえる(はず)</h1>
			<div>
